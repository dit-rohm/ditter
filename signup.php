...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  ...
  else if (strlen($screen_name) < 3 or strlen($screen_name) > 15) {
    $error['screen_name'] = '3文字以上15文字以下にしてください';
  }
  else if (screenNameExists($screen_name, $db)) {
  ...
  else if (emailExists($email, $db)) {
    $error['email'] = 'このメールアドレスは既に登録されています';
  }
  if (!preg_match("/^[a-zA-Z0-9]+$/", $password)) {
    $error['password'] = '英数字にしてください';
  }
  else if (strlen($password) < 4 or strlen($password) > 8) {
